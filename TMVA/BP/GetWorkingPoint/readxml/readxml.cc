...
//TH1F* hcountEffB = new TH1F("hcountEffB",Form(";%s;Background counts",mvatype.Data()),1000,-1.0,1.0);
	cout << "Pass 1.2" << endl;
	signal->Project("hcountEffS",Form("%s_pt_%.0f_%.0f",mvatype.Data(),ptmin,ptmax),(TCut(pthatweight.Data())*TCut(BptWeight.Data())*TCut(PVzWeight.Data())*TCut(weightfunctionreco.Data()))*(TCut(cuts.Data())));
...
//	cout << "hdisEffB->GetMaximum()*1.4 = " << hdisEffB->GetMaximum()*1.4 << endl;
	...
	*/
	...
	cfresult<<"  ----------------------------------------------------------"<<std::endl;
	cfresult<<" | "<<std::setiosflags(std::ios::left)<<std::setw(15)<<"Sig reg"<<" | "<<std::setw(9)<<Form("+- %.2f",dmassDsignal)<<" | "<<std::setw(13)<<"Sideband bkg"<<" | "<<std::setw(10)<<Form("%.0f",wBackground*(dmassDsidbandH-dmassDsidbandL)/(dmassDsignal*2))<<" |"<<std::endl;
	cfresult<<"  ----------------------------------------------------------"<<std::endl;
